 ...
 */
mongoose
  .connect(process.env.MONGODB_URI || "mongodb://localhost:27017/crypto-crash")
...
async function fetchCryptoPrices() {
  ...
  try {
    ...
    if (error.response?.status === 429) {
      console.log(
        "⚠️ Rate limited by CoinGecko, using cached prices for 60 seconds"
      );
      // Extend cache validity when rate limited
      priceCache.lastUpdate = Date.now() + 50000; // Use cache for 50 more seconds
    } else {
      console.error(
        "❌ Error fetching prices, using cached/fallback prices:",
        error.message
      );
      priceCache.lastUpdate = Date.now();
    }
...
async function endRound() {
  ...
  io.emit("roundCrash", {
    ...
    message: `Round crashed at ${crashPoint.toFixed(
      2
    )}x! Next round starting soon...`,
  });
...
app.get("/", (req, res) => {
  res.json({
    ...
    endpoints: [
      ...
      "GET /api/round/current - Current game round",
      "GET /api/round/history - Round history",
      "GET /api/player/:id/balance - Player balance",
      "POST /api/bet - Place a bet",
      "POST /api/wallet/add - Add balance (demo)",
      "POST /api/cashout - Cash out",
...
app.get("/api/player/:playerId/balance", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: error.message, balances: {} });
  }
});
...
app.post("/api/bet", async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ error: "USD amount must be between $0.01 and $10,000" });
    }
    ...
    if (!currency || !["BTC", "ETH", "USDT"].includes(currency)) {
      return res
        .status(400)
        .json({ error: "Currency must be BTC, ETH, or USDT" });
    }
...
app.post("/api/wallet/add", async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ error: "USD amount must be between $0.01 and $50,000" });
    }
    ...
    // Start atomic transaction
    await session.withTransaction(async () => {
      // Try to increment wallet for existing player
      let updatedPlayer = await Player.findOneAndUpdate(
        { playerId },
        { $inc: { [`wallets.${currency}`]: cryptoAmount } },
        { new: true, session }
      );
      // If player does not exist, create with correct wallet
      if (!updatedPlayer) {
        const wallets = {
          BTC: 0.001,
          ETH: 0.5,
        ...
        wallets[currency] += cryptoAmount;
        updatedPlayer = new Player({
          playerId,
          wallets,
          createdAt: new Date(),
        });
        await updatedPlayer.save({ session });
      }
...
app.post("/api/cashout", async (req, res) => {
  ...
  try {
    ...
    if (currentRound.status !== "active") {
      return res
        .status(400)
        .json({ error: "Round is not active - cannot cash out" });
    }
    ...
    if (currentMultiplier < 1) {
      return res
        .status(400)
        .json({ error: "Invalid game state - multiplier below 1x" });
    }
    ...
    if (currentMultiplier >= currentRound.crashPoint) {
      return res
        .status(400)
        .json({ error: "Round already crashed - cannot cash out" });
    }
    ...
    if (currentMultiplier >= currentRound.crashPoint * 0.98) {
      return res
        .status(400)
        .json({ error: "Too late to cash out - round about to crash" });
    }
...
app.get("/api/round/current", (req, res) => {
  if (currentRound) {
    res.json({
      ...
      crashPoint:
        currentRound.status === "crashed" ? currentRound.crashPoint : null,
      currentMultiplier:
        currentRound.status === "active" ? getCurrentMultiplier() : 1,
      timeElapsed:
        currentRound.status === "active" ? Date.now() - roundStartTime : 0,
...
app.get("/api/database/status", async (req, res) => {
  try {
    const status = {
      ...
      recentPlayers: await Player.find()
        .limit(5)
        .select("playerId createdAt wallets"),
      recentRounds: await GameRound.find()
        .sort({ startTime: -1 })
        .limit(5)
        .select("roundId status crashPoint startTime"),
      recentBets: await Bet.find()
        .sort({ timestamp: -1 })
        .limit(5)
        .select("playerId usdAmount currency cashedOut timestamp"),
      recentTransactions: await Transaction.find()
        .sort({ timestamp: -1 })
        .limit(5)
        .select("playerId transactionType usdAmount currency timestamp"),
    };

    console.log("📊 Database Status Check:", {
      connected: status.connection === "Connected",
      totalCollections: Object.values(status.collections).reduce(
        (a, b) => a + b,
        0
      ),
...
// --- ADMIN: Populate DB endpoint (should be protected in production!) ---
app.post("/api/admin/populate-db", async (req, res) => {
  try {
    await populateDatabase(Player, GameRound, Bet, Transaction);
    res.json({ success: true, message: "Database populated!" });
  } catch (error) {
    res.status(500).json({ success: false, error: error.message });
  }
});
...
io.on("connection", (socket) => {
  ...
  const recentConnections = Array.from(socketRateLimits.values()).filter(
    (limit) => Date.now() - limit.connectionTime < 5000
  ).length;
    ...
    try {
      ...
      if (suspiciousConnections.has(socket.id)) {
        socket.emit("cashoutError", {
          error: "Connection flagged for suspicious activity",
        });
        return;
      }
      ...
      if (now - limits.lastCashoutAttempt < 1000) {
        ...
        if (limits.cashoutAttempts > 3) {
          ...
          suspiciousConnections.add(socket.id);
          socket.emit("cashoutError", {
            error: "Rate limit exceeded - connection flagged",
          });
          socket.disconnect(true);
          return;
        }
        socket.emit("cashoutError", {
          error: "Rate limit: Please wait before next cashout attempt",
        });
        return;
      }
  ...
  socket.onAny((eventName, ...args) => {
    ...
    if (limits) {
      ...
      if (limits.messageCount > 200) {
        console.warn(
          "⚠️ Excessive messaging detected:",
          socket.id,
          "Event:",
          eventName
        );
